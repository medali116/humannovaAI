...
// Fonctions pour les interactions likes et commentaires (remplacement du localStorage par appels serveur)
function toggleLike(articleId) {
    fetch('article_interactions.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: new URLSearchParams({action: 'toggle_like', article_id: articleId})
    })
    .then(res => res.json())
    .then(data => {
        if (!data.error) {
            const block = document.querySelector(`[data-article-id="${articleId}"]`);
            const likeBtn = block.querySelector('.like-btn');
            const likesCount = block.querySelector('.likes-count');
...
function addComment(articleId, commentText) {
    ...
    .then(data => {
        if (!data.error) {
            ...
            const commentsCount = block.querySelector('.comments-count');
            const li = document.createElement('li');
            li.textContent = data.comment_text;
...
function loadArticleStatus(articleId) {
    fetch('article_interactions.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},})}
